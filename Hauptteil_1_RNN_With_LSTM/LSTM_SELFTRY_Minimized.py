...
def prepare_sequences(commands, channels, values1, durations, lookups, distincts, seq_len=32):
    ...
    network_input = [commands_network_input, channels_network_input, values1_network_input, durations_network_input]
    ...
    network_output = [commands_network_output, channels_network_output, values1_network_output, durations_network_output]

    return (network_input, network_output)

def create_network(n_commands, n_channel, n_value1, n_durations, embed_size=100, rnn_units=256, use_attention=False):
    ...
    x4 = Embedding(n_durations, embed_size)(durations_in)
    ...
    model = Model([commands_in, channel_in, value1_in, durations_in], [commands_out, channel_out, value1_out, durations_out])

    if use_attention:
        att_model = Model([commands_in, channel_in, value1_in, durations_in], alpha)
    else:
        att_model = None

    opti = RMSprop(lr=0.001)
    model.compile(loss=['categorical_crossentropy', 'categorical_crossentropy', 'categorical_crossentropy', 'categorical_crossentropy'], optimizer=opti)
