...
SETUP = {
    ...
    'training_data': r'.\training_data\lpd_17_Final_500_cleansed.npy',
    ...
    'prefix': '09_MultiInstrument_70_60',
...
TRAIN_CONFIG = {
    ...
    'two_stage_training': None,
    'training_phase': None,
    'num_epoch': 60,    #25 #50 pretrain, 30 train
...
MODEL_CONFIG = {
    ...
    # Parameters
    'batch_size': 8,  # Note: tf.layers.conv3d_transpose requires a fixed batch
    ...
    # Tracks
    'track_names': (
        'Drums', 'Piano', 'Chromatic Percussion', 'Organ', 'Guitar', 'Bass', 'Strings',
        'Ensemble', 'Brass', 'Reed', 'Pipe', 'Synth Lead', 'Synth Pad', 'Synth Effects', 'Ethnic',
        'Percussive', 'Sound Effects'
    ),
    'programs': (0, 0, 8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 88, 96, 104, 112, 120),
    'is_drums': (True, False, False, False, False, False, False, False, False,
    ...
    'metric_map': np.array([
        ...
        [True] * 17, # empty bar rate
        [True] * 17, # number of pitch used
        [False] + [True] * 16, # qualified note rate
        [False] + [True] * 16, # polyphonicity
        [False] + [True] * 16, # in scale rate
        [True] + [False] * 16, # in drum pattern rate
        [False] + [True] * 16  # number of chroma used
    ], dtype=bool),
    'tonal_distance_pairs': [(1, 4)], # pairs to compute the tonal distance
